...
const btnTranscript = $('transcript-btn');
...
chrome.storage.local.get(['mode', 'delay', 'count', 'deleteAll', 'deleteAllServer', 'channelId', 'guildId', 'fromId', 'untilId', 'order', 'language', 'transcript'], (res) => {
    if (res.mode === 'dm') {
        mode = 'dm';
    ...
    if (res.transcript !== undefined) {
        transcriptActive = res.transcript;
        btnTranscript.style.backgroundColor = transcriptActive ? '#43b581' : 'rgba(250, 166, 26, 0.8)';
    }
...
btnTranscript.addEventListener('click', () => {
    ...
    btnTranscript.style.backgroundColor = transcriptActive ? '#43b581' : 'rgba(250, 166, 26, 0.8)';
    chrome.storage.local.set({ transcript: transcriptActive });
    if (transcriptActive) {
        alert(t('transcript_warning'));
    }
});
...
btnStart.addEventListener('click', async () => {
    const settings = {
        ...
        i18n: {
            ...
            transcript_title: t('transcript_title'),
            transcript_info: t('transcript_info'),
